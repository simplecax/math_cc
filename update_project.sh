#!/bin/bash
#
# This script applies the definitive fix for the LinBox API, based on the
# ground truth provided by the user's diagnostic commands and file listings.
# It uses the correct headers, include order, and API calls.
#
# USAGE: ./final_fix.sh from the project root.

set -e

echo "🚀 Applying final, evidence-based API corrections to atopo.h..."
...
namespace atopo {
    ...
    // --- TRAITS ---
    template<typename CellType> struct CellTraits { /* ... */ };
    template<typename TopologySource> struct TopologySourceTraits { /* ... */ };
...
# Restore omitted sections in inc/atopo.h
sed -i '/template<typename CellType> struct CellTraits {/c\
...
sed -i '/template<typename TopologySource> struct TopologySourceTraits {/c\
    template<typename TopologySource>\
    struct TopologySourceTraits { /* User specializes */ };' inc/atopo.h

echo "✅ All include paths have been corrected based on your file listings."
echo "   Please clean and rebuild. This should be the final step."
echo "   Run: rm -rf build && cmake -S . -B build && cmake --build build"